...
// Return: User{ UserID, FirstName, LastName, etc.}
...
app.get('/user/role', async (req, res) => {
  ...
  if(err_userData) {
    return res.status(404).json({ error: `Failed to Query User ${userID}'s Role`, message: err_userData.message});
  }
  let role = userData[0];
  if(!role)
    return res.status(404).json({ error: `User ${userID}'s Role Not Found`})
