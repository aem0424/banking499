...
function calculateMortgagePayment(interestRate, years, baseCost) {
  const rate = interestRate / 100 / 12;
  const n = years * 12;
  const monthlyPayment = (baseCost * rate) / (1 - Math.pow(1 + rate, -n));
...
router.get('/system/mortgage', async (req, res) => {
  let userRole = req.session.user?.Role;
  // if (userRole != "Administrator") return res.status(401).json({ error: "User Is Not Logged In As Admin" });
  try {
    const result = await serviceMortgageAccounts();
    res.json({ success: true, result });
  } catch (error) {
...
// Params: None
// Return: None
async function serviceMortgageAccounts() {
  try {
    const [data, error] = await database.getOverdueMortgagePayments();
...
module.exports = {
  ...
  serviceMortgageAccounts: serviceMortgageAccounts
};
